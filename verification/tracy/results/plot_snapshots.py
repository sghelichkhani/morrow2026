"""3x3 snapshot panel of the Tracy 2D solution.

Reads the time-series VTK collection written by the Tracy driver
(solution.pvd + solution/*.vtu in this directory) and builds a 3x3 figure:
...
shared along each row, so the three times are directly comparable.

Run from the Firedrake venv (pyvista lives there); no gadopt import needed:

    ~/Workplace/firedrake-2026-03-03/venv-firedrake/bin/python3 plot_snapshots.py
"""
...
PVD = HERE / "solution.pvd"
OUT = HERE / "tracy_snapshots.png"
...
def main():
    ...
    for r, (field, row_label, bar_label, cmap, log) in enumerate(FIELDS):
        ...
        for c, mesh in enumerate(meshes):
            ...
            # Panel letter (A, B, C, ...) in a light-green circle, top-left.
            ax.text(
                0.04,
                0.96,
                chr(ord("A") + r * ncols + c),
